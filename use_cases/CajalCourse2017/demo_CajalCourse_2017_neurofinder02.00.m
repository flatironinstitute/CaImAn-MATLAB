...
%%  play raw movie 
implay((Y-minY)/(maxY-minY),100)

% figure;
%     for t = 1:T
%         imagesc(Y(:,:,t),[minY,maxY]); axis square; axis off; colormap('gray');
%         title(['Raw data, frame ',num2str(t),' out of ',num2str(T)]);
%         pause(0.001);
%     end
...
% you might want to avoid to use the implay command since there is no text with the movies...
implay(cat(2,Y_sub,M_rgs)/maxY)
% figure;
%     for t = 1:size(Y_sub,3)
%         subplot(121); imagesc(Y_sub(:,:,t),[minY,maxY]); axis square; axis off; colormap('gray');
%                       title(['Raw data (downsampled), frame ',num2str(t)]);
%         subplot(122); imagesc(M_rgs(:,:,t),[minY,maxY]); axis square; axis off; colormap('gray');
%                       title('Rigidly corrected data (downsampled)');
%         pause(0.001);
%     end
...
options_nr = NoRMCorreSetParms('d1',size(Y,1),'d2',size(Y,2),...
                    ...
                    'bin_width',100,'max_shift',15,'max_dev',8,'us_fac',50);
...
implay(cat(2,Y_sub,M_rgs,M_nrs)/maxY)

% figure;
%     for t = 1:size(Y_sub,3)
%         subplot(131); imagesc(Y_sub(:,:,t),[minY,maxY]); axis square; axis off; colormap('gray');
%                       title('Raw data');
%         subplot(132); imagesc(M_rgs(:,:,t),[minY,maxY]); axis square; colormap('gray');
%                       title('Rigidly corrected');
%                       set(gca,'Xtick',[],'Ytick',[]);
%                       xlabel(['Frame ',num2str(t)]);
%         subplot(133); imagesc(M_nrs(:,:,t),[minY,maxY]); axis square; axis off; colormap('gray');
%                       title('Piecewise-rigidly corrected');
%         pause(0.001);
%     end
