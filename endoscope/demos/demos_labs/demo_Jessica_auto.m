...
for miter=1:maxIter
    ...
    if miter<2
        merge_thr = [0.0, 0.5, 0];     % thresholds for merging neuron (spatial overlap, temporal correlction, spike correlation)
    else
        merge_thr = [0.3, 0.6, 0];
    end
...
temp  = quantile(Y(1:1000:(d1*d2*T)), [0.0001, 0.9995]);
Ymin = temp(1); 
Ymax = temp(2); 
ACmax = quantile(Yac(1:1000:(d1*d2*T)), 0.9995);
