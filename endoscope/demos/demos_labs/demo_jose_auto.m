...
% nam = '~/Dropbox/inscopix_imaging data for pc/CaMKII_120_TMT Exposure_5fps.tif'; 
...
tau_rise = 0.1; 
nframe_decay = ceil(tau_decay*10*neuron_raw.Fs); 
bound_pars = false; 
...
patch_par = [3,3]; %1;  % divide the optical field into m X n patches and do initialization patch by patch
K = 300; % maximum number of neurons to search within each patch. you can use [] to search the number automatically
neuron.options.bd = 3; % boundaries to be removed due to motion correction
...
for miter=1:5
    ...
    if miter==1
        merge_thr = [0.0, 0.8, 0];     % thresholds for merging neurons corresponding to
    else
        merge_thr = [0.1, 0.6, 0];
    end
    ...
    with_dendrites = true;
    if with_dendrites
        neuron.options.se = strel('disk', 20);
        IND = determine_search_location(neuron.A, 'dilate', neuron.options);
    else
        neuron.options.dist = 3;  %#ok<UNRCH>
...
neuron.viewContours(Cnn, 0.7, 0); % correlation image computed with background-subtracted data
colormap winter;
axis equal; axis off;
...
temp  = quantile(Y(1:100:numel(Y)), [0.0001, 0.9999]);
Ymin = temp(1); 
Ymax = temp(2); 
ACmax = quantile(Yac(1:100:numel(Yac)), 0.99999);
...
for m=1:2:T
    ...
    %     imagesc(Ybg(:, :, m), [-50, 50]);
    text(d2s/2-30, 10, sprintf('Time: %.2f Sec', m/neuron.Fs), 'color', 'w');
