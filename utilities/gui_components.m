function gui_components(rval_space,rval_time,max_pr,sizeA,Am,Cn,options)

%[rval_space,rval_time,max_pr,sizeA,keep] = classify_components(Y,A,C,b,f,YrA,options);

%%
figure;CC = plot_contours(Am,Cn,options,0); title('Selected components','fontweight','bold','fontsize',14); close all;
%%
plt_cont = @(x) plot_contours(Am,Cn,options,0,[],CC,[],x);
%%
d1 = options.d1;
d2 = options.d2;
